...
def _get_walk_script(id):
    s = Script(uid='_main')
    has_item = vars.inventory.get(id, 0) > 0
    if not has_item:
        item = vars.items[id]
        wt = item.get('walkto', None)
        if wt is not None:
...
open_ = say(['$defaultactions/1'])
lookat_ = say(['$defaultactions/4'])
pickup_ = say(['$defaultactions/5'])

lookat_meat = say(['$lines/35'])
lookat_pot = say(['$lines/36'])
lookat_fish = say(['$lines/37'])
