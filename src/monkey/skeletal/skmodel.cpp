...
SkModel::SkModel(const ITable & t) {
    ...
    t.foreach<PyDict>("polygons", [&] (const PyDict& po) {
        ...
        for (unsigned int i = 0 ; i < points.size(); i += point_size) {
            int offset = point_size * i;
            VertexSkeletal vertex;
            //auto tup = po[i].cast<pybind11::tuple>();
            vertex.x = points[offset];
            vertex.y = points[offset+1];
            vertex.z = points[offset+2];
            if (autotc) {
                vertex.s = vertex.x / texw;
                vertex.t = vertex.y / texh;
            } else {
                vertex.s = points[offset + 3] / texw;
                vertex.t = points[offset + 4] / texh;
            }

            vertex.index0 = points[offset+5];
            vertex.index1 = points[offset+6];
            vertex.index2 = points[offset+7];
            vertex.weight0 = points[offset+8];
            vertex.weight1 = points[offset+9];
            vertex.weight2 = points[offset+10];
    ...
    t.foreach<PyDict>("joints", [&] (const PyDict& dict) {
        ...
        auto joint = std::make_shared<Joint>(curr++, id, glm::mat4(1.0f));
        if (root) {
            m_rootJoint = joint;
        }
    });
