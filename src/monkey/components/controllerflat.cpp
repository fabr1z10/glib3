...
ControllerFlat::ControllerFlat(const ITable &t) {
	...
	m_horizontalRaySpacing = m_size[1] / (m_horizontalRayCount - 1);
	m_verticalRaySpacing = m_size[0] / (m_verticalRayCount - 1);
}
...
void ControllerFlat::HorizontalCollisions(glm::vec2 &velocity) {
	...
	vec2 r0 = pos + glm::vec2(facingLeft ? -m_size[0] : m_size[0], 0.0f);
...
void ControllerFlat::VerticalCollisions(glm::vec2& velocity) {
	...
	glm::vec2 pos = m_entity->GetPosition();
	vec2 r0 = pos + vec2(0, (directionY == -1) ? -m_size[1] : m_size[1]);
