...
void TextView::AddEntity(const std::string & text) {
    ...
    auto ptr = mf->GetShared<Entity>(f);
...
void TextView::Start() {
    ...
    m_entity->SetCamera(cam);
    //m_entity->onAdd.Register(this, [&] (Entity* e) { this->notifyAddChild(e); });
    cam->SetPosition(glm::vec3(m_loc.x + m_width*0.5f, m_loc.y-m_height*0.5f, 5.0f), glm::vec3(0,0,-1), glm::vec3(0,1,0));
    //m_nextPos = glm::vec2(m_loc.x, m_loc.y + m_height);
    //UpdateCamPosition(0);<
}
