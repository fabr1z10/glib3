...
extern "C" void hello(SceneFactory* f) {
    ...
    f->addStateFactory("bubble", std::make_unique<BubbleFactory>());
...
std::shared_ptr<State> BubbleFactory::Create(luabridge::LuaRef &ref) {

    LuaTable table(ref);
    bool left = table.Get<bool>("left");
    float speed = table.Get<float>("speed");
    float speedUp = table.Get<float>("speedup");
    return std::make_shared<Bubble>(left,speed,speedUp);
