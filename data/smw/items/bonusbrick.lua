...
items.bonusbrick.create = function(arg)
	local pos = arg.pos
	local sprite = arg.sprite or "bonusbrick"
	...
	return {
		...
		children = {
			-- head sensor
			{
				pos = { 0, -0.25, 0},
				components = {
					{ type="collider", shape = s1, tag = bonusBrickTag, flag = collisionFlags.enemy, mask = collisionFlags.player },
...
function mario_bonusbrick(e1, e2)
	print ("Hit a bonus brick")
